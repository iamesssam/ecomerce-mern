...
const Navbar = () => {
    ...
    return (
        <>
            <nav className='container mx-auto flex items-center justify-between py-4 px-6'>
                ...
                <div className='flex items-center space-x-4'>
                    ...
                    <button className='relative hover:text-black'>
                        <HiOutlineShoppingBag className='w-6 h-6 text-gray-700 cursor-pointer'
                            onClick={toggleCart}
                        />
                        {cartItemCount > 0 && (
            ...
            <CartDrawer isOpen={isOpen} toggleCart={toggleCart} />

            {/* Mobile Navigation */}
            <div className={`fixed bg-white top-0 left-0 w-3/4 sm:w-1/2 md:w-1/3 shadow-lg 
                transform transition-transform duration-300 z-50 h-full
                ${navDrawerOpen ? "translate-x-0" : "-translate-x-full"}
                `}>
                <div className='absolute right-2'>
                    <button>
                        <IoMdClose className='w-6 h-6 text-gray-800 mt-3'
